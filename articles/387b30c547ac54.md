...
このコマンドは、マイグレーションファイルが生成されていない場合、マイグレーションファイルを生成した後、データベースに適用し、スキーマの変更を反映させます。
...
適用したマイグレーションを取り消す（ロールバックする）場合は、下記コマンドを使用します。このコマンドは、履歴を見て、データベースをマイグレーション前の状態に戻します。
...
async function createPostWithTransaction(title: string, content: string) {
  try {
...
ブラウザから、[TODO：localhost](https://www.prisma.io/studio)にアクセスすると、User 情報を取得できると思います。
TODO: 図
