...
適用したマイグレーションを取り消す（ロールバックする）場合は、下記コマンドを使用します。このコマンドは、履歴を見て、データベースをマイグレーション前の状態に戻します。詳細については、公式ドキュメント等を参考にして実行してください。
...
ブラウザから、[TODO：localhost](https://www.prisma.io/studio)にアクセスすると、User 情報を取得できます。
