...
2. **ポート 9222 でブラウザを別途起動する手間**。`--remote-debugging-port=9222` をつけて Chrome を起動するか、常駐させておく必要がある。
3. **認証セッションが引き継げない**。社内システムや SSO が必要なサービスは、毎回ログインし直しになる。
...
- 2: [社内システム](https://internal.example.com/...)
```

SSO 突破なしで社内ページにアクセスできます。これが一番うれしいところです。
...
| 認証     | 毎回必要                             | 引き継ぎできる             |
| 設定     | ツールごとに MCP 設定が必要          | インストールするだけ       |
