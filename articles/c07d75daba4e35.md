...
みなさん、観測してますか 🔭？OpenTelemetry（OTEL: ベンダーに依存しないテレメトリ標準）をきちんと理解したくて、最小の Todo API を題材に、ローカルで Grafana LGTM（Loki: ログ、Grafana: 可視化、Tempo: トレース、Prometheus/Mimir: メトリクス） スタックを利用し観測基盤を組み、コードを触らずにトレース・ログ・メトリクスを観測できるかを確認した記録です。
