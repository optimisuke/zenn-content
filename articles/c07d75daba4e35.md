...
みなさん、観測してますか 🔭？OpenTelemetry（OTEL: ベンダーに依存しないテレメトリ標準）をきちんと理解したくて、最小の Todo API を題材に、ローカルで Grafana LGTM（Loki: ログ、Grafana: 可視化、Tempo: トレース、Prometheus/Mimir: メトリクス） スタックを利用し観測基盤を組み、コードをほぼ触らずにトレース・ログ・メトリクスを観測できるかを確認した記録です。
...
アプリから collector に OTLP を集約し、そこで spanmetrics を生成して LGTM に流す構成にすると、送信先や集約ロジックを後から差し替えやすい。

## まとめ

コードを触らずに OTEL でトレース/ログ/メトリクスを集約できることを確認できました。
間違い等あれば、ご指摘お願いします。
