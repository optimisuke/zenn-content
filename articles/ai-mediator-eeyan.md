...
この体験をもとに、生成AIを使ったアプリを開発・実証し、デザイン学研究作品集（JSSD 2025年）に論文として発表しました。その後、Zenn のMicrosoft Agent Hackathon を見つけたことをきっかけに、アプリをゼロから作り直しました。背景にある活動は数年来のものですが、技術的には完全にリビルドしました。今回は、インフラ構築から CLI・Terraform・コーディングまで、ほぼすべてをコーディングエージェント（Claude Code と Codex）でやりきる、という制約で進めました。Azure は初体験でしたが、なんとかなりました。
...
あわせて、今回はインフラ構築・CLI・Terraform・コーディングを含めほぼすべての作業をコーディングエージェント（Claude Code と Codex）でやりきる、という方針で進めました。Azure は自分にとって初体験でしたが、他のクラウドを使ったことがあれば「同じようなサービスがあるだろう」という感覚で手探りしながらなんとかなりました。詰まったポイントや気づきは後半の「コーディングエージェントと一緒に作った」でまとめています。
...
![ええやん！.ai の Azure アーキテクチャ。Next.js を Azure Container Apps で動かし、Azure OpenAI、PostgreSQL、Blob Storage、Application Insights と連携する](/images/ai-mediator-eeyan/azure-architecture.png)
